...
func minInt(x uint32, y uint32) uint32 {
	if x < y {
		return x
	} else {
		return y
...
func minimizeTextures(ctx context.Context) transform.Transformer {
	...
	return transform.Transform("Minimize textures", func(ctx context.Context,
		...
		case *VkCmdCopyBufferToImage:
			...
			width := minInt(imageExtent.Width(), newTexWidth)
			height := minInt(imageExtent.Height(), newTexHeight)
			depth := minInt(imageExtent.Depth(), newTexDepth)
